...
var ApiClient = function () {
  ...
  */
	function ApiClient() {
		...
		this.freq = params.requestsFrequency ? typeof params.requestsFrequency === "number" ? params.requestsFrequency : 333 : null;
	...
	_createClass(ApiClient, [{
		...
		value: function call(method, params, callback) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (_this.freq) {
					var now = Date.now(),
					    diff = now - _this.lastRequestTime;

					if (diff < _this.freq) {
						timeout = _this.freq - diff;
					}
					_this.lastRequestTime = now + timeout;
				}
				...
				setTimeout(function () {
					...
					if (_this.extraParams) {
						var key = void 0;
						for (key in _this.extraParams) {
							if (_this.extraParams.hasOwnProperty(key) && !params.hasOwnProperty(key)) params[key] = _this.extraParams[key];
						}
					}
					...
					var query = paramsToQueryString(params),
					    url = _this.endpoint + method + (query.length && '?' + query || "");

					_this.fetch(url, { credentials: 'same-origin' }).then(json).then(function (res) {
						if (callback) {
